...
class _WP_Field_Renderer {
	...
	public static function render_text_field ( $settings, $echo = true ) {
        ...
        if (isset($settings['i18n'])) {
            $settings['desc'] = __($settings['i18n'], 'rent-rabbit');
        }
		...
		<fieldset>
			...
			<label for="<?php echo esc_attr( $id ); ?>">
				...
				<input
					...
					<?php if (!empty($settings['disabled'])) : ?>
					    disabled
					<?php endif;?>
				>
			...
			<label for="<?php echo esc_attr( $id ); ?>">
				...
				<div class="checkboxes">
					...
					<input
						...
						<?php empty( $value ) ? '' : checked( in_array( $choice_value, $value )); ?>
					>
	...
	public static function render_image_field ( $settings, $echo = true ) {
		...
		<p class="description">
		<?php self::render_description( $settings['desc'] ); ?>
		</p>
